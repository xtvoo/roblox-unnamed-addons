...
local MainTab = api:GetTab("main") or api:AddTab("main")
local RageTab = api:GetTab("ragebot") or api:AddTab("ragebot")
local WorldTab = api:GetTab("world") or api:AddTab("world")
local VisualsTab = api:GetTab("visuals") or api:AddTab("visuals")
local CharacterTab = api:GetTab("character") or api:AddTab("character")
local MiscTab = api:GetTab("misc") or api:AddTab("misc")
local SettingsTab = api:GetTab("settings") or api:AddTab("settings")

local Main = MainTab:AddLeftGroupbox('Seth Main')
local Rage = RageTab:AddRightGroupbox("Seth Rage")
local Spawn = WorldTab:AddRightGroupbox('Seth Spawn')
local Grip = VisualsTab:AddRightGroupbox('Seth Grip')
local Character = CharacterTab:AddLeftGroupbox("Seth Character")
local Shop = MiscTab:AddLeftGroupbox("Seth Shop")
local LibraryThing = MiscTab:AddLeftGroupbox("Seth Library")
local Notification = SettingsTab:AddLeftGroupbox("Seth Notifications")
local Misc = MiscTab:AddLeftGroupbox("Seth Miscellaneous")
...
-- Helper function from BringPlayer.lua to get correct body part
local function resolve_upper_root(plr)
    local char = plr.Character
    if not char then return nil, nil end
    -- Prioritize RootPart for stability, fallback to Torso/UpperTorso for ragdolled
...
local VoidBox = CreateDependency("VoidBox", Rage, Toggles["VoidEnabled"])

VoidBox:AddDropdown('Void Dropdown', {
    Values = { 'nan', 'up down', 'spun', 'wierd', 'chaos', 'glitch' },
    Default = 1,
})
    ...
    if Toggles and Toggles["HudEnabled"] and Toggles["HudEnabled"].Value and LocalPlayer and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") and LocalPlayer.Character:FindFirstChild("Humanoid") then
        ...
        if BringActive then
            if Target and Target.Character and Target.Character:FindFirstChild("HumanoidRootPart") and Target.Character:FindFirstChild("Humanoid") and Handler:Is_KO(Target) and Handler:Is_KO(Target).Value then
                ...
                BringActive = true
                local OriginCF = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") and LocalPlayer.Character.HumanoidRootPart.CFrame
                if not OriginCF then
                    Notify("Failed to save origin position")
                    BringActive = false
                    return
                end
                local GrabStart = tick()
                local Phase = "GRAB" -- GRAB -> RETURN
                
                Handler:AddConnection("BringLoop", RunService.Heartbeat:Connect(function()
                    -- Force fresh target reference
                    if not Target or not Target.Parent then
                        BringActive = false
                        Handler:Disconnect("BringLoop")
                        return
                    end
                    ...
                    local isKOd = Handler:Is_KO(Target) and Handler:Is_KO(Target).Value
                    
                    if targetRoot and isKOd then
                        -- Physics Glue
                        pcall(function()
                            if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                                sethiddenproperty(LocalPlayer.Character.HumanoidRootPart, "PhysicsRepRootPart", targetRoot)
                            end
                        end)
                        
                        if Phase == "GRAB" then
                            -- Phase 1: Glue with CFrame (auto stomp pattern)
                            local Offset = Options and Options["ragebot_stomp_offset"] and Options["ragebot_stomp_offset"].Value or 3
                            local base_cf = targetRoot.CFrame
                            local torso_pos = base_cf.Position
                            local pos = torso_pos + Vector3.new(0, 1, 0) * Offset
                            
                            local look = base_cf.LookVector
                            local flat_look = Vector3.new(look.X, 0, look.Z)
                            if flat_look.Magnitude * flat_look.Magnitude < 0.01 then
                                flat_look = Vector3.new(0, 0, -1)
                            else
                                flat_look = flat_look.Unit
                            end
                            
                            local desyncCF = CFrame.new(pos, pos + flat_look)
                            if api.can_desync and api:can_desync() then
                                api:set_desync_cframe(desyncCF)
                            end
                            if api.set_server_cframe then
                                api:set_server_cframe(desyncCF)
                            end
                            
                            if tick() - GrabStart > 1.0 then
                                Phase = "RETURN"
                                Notify("Returning to origin")
                            end
                        elseif Phase == "RETURN" then
                            -- Phase 2: Return to origin (dragging them)
                            if OriginCF then
                                if api.can_desync and api:can_desync() then
                                    api:set_desync_cframe(OriginCF)
                                end
                                if api.set_server_cframe then
                                    api:set_server_cframe(OriginCF)
                                end
                            else
                                Notify("No origin saved, stopping")
                                BringActive = false
                            end
                        end
                    ...
                    if not Target or not targetChar or targetChar:FindFirstChild('GRABBING_CONSTRAINT') or not isKOd or not BringActive then
                        BringActive = false
                        Handler:Disconnect("BringLoop")
                        pcall(function()
                            if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                        ...
                        Notify(`bringed {Target and Target.Name or "target"}`)
                    end
                end))
            else
                if LocalPlayer and LocalPlayer.Character and LocalPlayer.Backpack or LocalPlayer.Character:FindFirstChildWhichIsA("Tool") then
