...
local function Loop()
    ...
    -- Set desync position with offset (matches glue addon pattern)
    -- Position in front of target using offset
    local offset = Vector3.new(0, 0, -Config.Distance)
    local desyncCFrame = CFrame.new(targetRoot.Position + offset)
