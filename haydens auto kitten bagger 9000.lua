...
Bagging:AddDropdown("BagStrategy", {
    Values = {"Adaptive", "Behind", "Below", "Above", "Aggressive Spin", "Custom"},
    Default = 1,
    Text = "Bag Position Strategy",
    Tooltip = "Adaptive tries multiple positions"
})

Bagging:AddToggle("BagCustomToggle", { Text = "Custom Positions", Default = false, Visible = false })

CreateDependency("CustomBagBox", Bagging, Toggles.BagCustomToggle)

CustomBagBox:AddSlider("BagCustomX", { Text = "Custom X Offset", Default = 0, Min = -10, Max = 10, Rounding = 1, Suffix = " studs" })
CustomBagBox:AddSlider("BagCustomY", { Text = "Custom Y Offset", Default = 0, Min = -10, Max = 10, Rounding = 1, Suffix = " studs" })
CustomBagBox:AddSlider("BagCustomZ", { Text = "Custom Z Offset", Default = 0, Min = -10, Max = 10, Rounding = 1, Suffix = " studs" })
CustomBagBox:AddSlider("BagCustomRotX", { Text = "Custom Rot X", Default = 0, Min = -180, Max = 180, Rounding = 0, Suffix = "°" })
CustomBagBox:AddSlider("BagCustomRotY", { Text = "Custom Rot Y", Default = 0, Min = -180, Max = 180, Rounding = 0, Suffix = "°" })
CustomBagBox:AddSlider("BagCustomRotZ", { Text = "Custom Rot Z", Default = 0, Min = -180, Max = 180, Rounding = 0, Suffix = "°" })

Options.BagStrategy:OnChanged(function()
    Toggles.BagCustomToggle:SetValue(Options.BagStrategy.Value == "Custom")
end)
...
Safety:AddToggle("AntiBag", {
    Text = "Anti Bag",
    Default = true,
    Tooltip = "Reset character if you get bagged"
})
...
local function IsBagged(target)
    ...
    local isTargetLocal = (target == LocalPlayer)
    local char = target.Character
    
    -- Check character children directly first (faster for local player)
    if char:FindFirstChild("Christmas_Sock") or char:FindFirstChild("BrownBag") or char:FindFirstChild("Bag") then
        return true
    end

    -- Fallback to the same way we check others (Workspace.Players.Model)
    local TargetModel = Workspace.Players.Model:FindFirstChild(target.Name)
    if TargetModel then
        if TargetModel:FindFirstChild("Christmas_Sock") or TargetModel:FindFirstChild("BrownBag") or TargetModel:FindFirstChild("Bag") then
            return true
        end
    end
...
local function TriggerBuyBag()
    if not (Toggles.MasterSwitch and Toggles.MasterSwitch.Value) then return end
    if api:can_desync() and not State.BuyingBag then
        ...
        if rbStatus ~= "buying" and (ignoreCooldown or currentTime - State.LastBuyTime > 2) then
            ...
            task.spawn(function()
                ...
                api:buy_item("brownbag", false, true) -- Double tap instant
                
                task.wait()
                -- Uncrouch
                vim:SendKeyEvent(false, Enum.KeyCode.LeftControl, false, game)
                
                task.wait(0.1) -- Short cooldown
...
local function UseBag()
    if not (Toggles.MasterSwitch and Toggles.MasterSwitch.Value) then return false end
    local currentTime = tick()
    ...
    if Bag.Parent == LocalPlayer.Backpack then
        ...
        if humanoid then
            ...
            -- Instant activate attempt same frame
            task.spawn(function()
                 Bag:Activate()
            end)
        end
...
local function GetBagPosition(targetRoot)
    ...
    elseif strategy == "Above" then
        -- Strategy #7 is "Above" (index 7 in the table)
        return BagStrategies[7](targetRoot)
    elseif strategy == "Aggressive Spin" then
        -- Strategy #6 is "Aggressive Spin" (index 6 in the table)
        return BagStrategies[6](targetRoot)
    elseif strategy == "Custom" then
        local ox = Options.BagCustomX and Options.BagCustomX.Value or 0
        local oy = Options.BagCustomY and Options.BagCustomY.Value or 0
        local oz = Options.BagCustomZ and Options.BagCustomZ.Value or 0
        local rx = Options.BagCustomRotX and Options.BagCustomRotX.Value or 0
        local ry = Options.BagCustomRotY and Options.BagCustomRotY.Value or 0
        local rz = Options.BagCustomRotZ and Options.BagCustomRotZ.Value or 0
        
        return targetRoot.CFrame * CFrame.new(ox, oy, oz) * CFrame.Angles(math.rad(rx), math.rad(ry), math.rad(rz))
    end
...
api:add_connection(RunService.Heartbeat:Connect(function(dt)
    ...
    if State.Target and (IsTargetValid(State.Target) or IsDead(State.Target)) then
        ...
        if not wasBagged and State.IsBagged then
            ...
            if Toggles.DeepDebug and Toggles.DeepDebug.Value then
                ...
                local report = string.format(
                    ...
                    "> TOTAL SEQUENCE:    %.4fs\n" ..
                    "------------------------------------------------",
                    (State.Target and State.Target.Name or "Unknown"), math.max(0, r_react), math.max(0, r_input), math.max(0, r_ping), math.max(0, r_total)
                )
        ...
        if isDead and Toggles.VoidWhenDead and Toggles.VoidWhenDead.Value then
                ...
                if State.Mode ~= "void_dead" then
                    ...
                    State.ShouldVoid = true
                    api:set_ragebot(false)
                    SetFlameMode(false)
                    DebugLog("Target dead - Voiding...")
                end
                
                if State.ShouldVoid then
                    pcall(function()
                        LocalPlayer.Character.HumanoidRootPart.CFrame = VoidCFrame
                    end)
                end
                return 
        end
        ...
        -- NORMAL LOGIC (Kill/Bag/Stomp)
        -- NORMAL LOGIC ( Kill / Bag / Stomp )
        -- This block was previously corrupted.
        ...
        if shouldStomp then
            ...
            if State.Target then
                UpdateTargetUI(State.Target.Name or "Unknown")
            end
            SetFlameMode(false)
    ...
    if not success then
        warn("[AutoBag Error]: " .. tostring(err))
    end
end))
...
local PhysicsConnection = api:add_connection(RunService.Heartbeat:Connect(function()
    if not (Toggles.MasterSwitch and Toggles.MasterSwitch.Value) or not State.Target then 
        if Toggles.AntiBag and Toggles.AntiBag.Value and IsBagged(LocalPlayer) then
