...
-- ==================== UI SETUP ====================
local tabs = {
    Main = api:AddTab("Main"),
    Addons = api:AddTab("Addons")
}
...
-- ==================== AI AGENT TAB ====================
local aiBox, ai_enable, ai_training, ai_explore, ai_status_state, ai_status_action
local aiStatsBox, ai_total_reward, ai_qtable_size

if tabs.AI then
    aiBox = tabs.AI:AddLeftGroupbox("Aegis Control")
    ai_enable = aiBox:AddToggle("aegis_enable", { Text = "Auto-Pilot", Default = false })
    ai_training = aiBox:AddToggle("aegis_training", { Text = "Training Mode", Default = true })
    ai_explore = aiBox:AddSlider("aegis_explore", { Text = "Exploration Rate", Default = 0.2, Min = 0, Max = 1, Rounding = 2 })
    ai_status_state = aiBox:AddLabel("State: Unknown")
    ai_status_action = aiBox:AddLabel("Action: None")

    aiStatsBox = tabs.AI:AddRightGroupbox("Learning Stats")
    ai_total_reward = aiStatsBox:AddLabel("Total Reward: 0")
    ai_qtable_size = aiStatsBox:AddLabel("Q-Table States: 0")

    aiStatsBox:AddButton("Save Training", function()
        if Aegis then
            Aegis:Save()
            api:notify("Aegis training data saved!", 2)
        end
    end)

    aiStatsBox:AddButton("Reset Q-Table", function()
        if Aegis then
            Aegis.QTable = {}
            Aegis.TotalReward = 0
            api:notify("Q-Table reset!", 2)
        end
    end)

    -- Link toggle to Aegis
    if ai_enable then
        ai_enable:OnChanged(function(val)
            if Aegis then Aegis.Enabled = val end
        end)
    end

    if ai_training then
        ai_training:OnChanged(function(val)
            if Aegis then Aegis.Training = val end
        end)
    end
end
...
-- ==================== AI DECISION LOOP ====================
local aiLastTick = 0
local aiConn = RunService.Heartbeat:Connect(function()
    if not Aegis or not Aegis.Enabled then return end
    
    local now = os.clock()
    if now - aiLastTick < 1 then return end -- Run every 1 second
    aiLastTick = now
    
    -- Gather enemy data
    local enemies = {}
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= LocalPlayer and not isKnocked(p) and not isDead(p) then
            table.insert(enemies, p)
        end
    end
    
    -- Track health changes for damage taken
    local hum = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid")
    if hum then
        local currentHealth = hum.Health
        if currentHealth < aiPerformance.LastHealth then
            aiPerformance.DamageTaken = aiPerformance.LastHealth - currentHealth
        else
            aiPerformance.DamageTaken = 0
        end
        aiPerformance.LastHealth = currentHealth
    end
    
    -- Run AI Step
    local action, currentState = Aegis:Step(LocalPlayer, enemies, aiPerformance)
    
    -- Reset performance flags after consumption
    aiPerformance.Hit = false
    aiPerformance.Kill = false
    aiPerformance.Death = false
    
    if action then
        -- Apply action to script settings
        if strafe_mode and strafe_mode.SetValue then
            pcall(function() strafe_mode:SetValue(action.Strafe) end)
        end
        if strafe_speed and strafe_speed.SetValue then
            pcall(function() strafe_speed:SetValue(action.Speed) end)
        end
        if dist_enable and dist_enable.SetValue then
            pcall(function() dist_enable:SetValue(action.DistSpam) end)
        end
        
        -- Update UI labels
        if ai_status_state and ai_status_state.Text then
            ai_status_state.Text = "State: " .. tostring(currentState)
        end
        if ai_status_action and ai_status_action.Text then
            ai_status_action.Text = "Action: " .. tostring(action.Name)
        end
        
        -- Update stats
        local qCount = 0
        for _ in pairs(Aegis.QTable) do qCount = qCount + 1 end
        if ai_qtable_size then ai_qtable_size.Text = "Q-Table States: " .. qCount end
    end
end)
api:add_connection(aiConn)
